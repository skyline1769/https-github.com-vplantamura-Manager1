...
<div id="crumb" class="clearBorder">当前位置<span class="to"></span><a href="action/global/index">系统概览</a><span class="to"></span><a href="action/global/home">我的主页</a><span class="to"></span><a href="action/global/advance">高级管理</a><span class="to"></span><a href="action/data/filter">浏览配置</a><span class="to"></span>浏览备用配置</div>
<div id="main-container-outter">
	<div id="main-container-inner">
		<div class="noteword"><b>温馨提示</b>：以下展示的是系统中全部备用配置数据</div>
		...
		<div class="box">
				<c:choose>
					...
					<form action="action/bak/batchDelete">
						<input type="hidden" name="location" value="advance" />
							...
							<c:forEach var="model" items="${dataList}" varStatus="status">
								<c:choose>
									...
									<tr>
										...
										<td class="bbottom">
											<a href="action/bak/show?id=${model.id }&dataid=${dataid}" title="查看详细备用配置">${model.description}</a>
										</td>
										<td class="bbottom"><fmt:formatDate value="${model.addtime }" type="both"/></td>
										<td class="bright bbottom">
											<a href="action/bak/show?id=${model.id }&dataid=${dataid}" class="detail">查看</a>
											<a href="action/bak/viewModify?id=${model.id }&location=advance" class="modify">修改</a>
										</td>
									</tr>
									</c:when>
									<c:otherwise>
